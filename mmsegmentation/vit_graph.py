...
from mmcv.cnn.utils.weight_init import (constant_init, kaiming_init,
                                        trunc_normal_)
from mmcv.runner import (BaseModule, CheckpointLoader, ModuleList, Sequential,
                         load_state_dict)
...
class VisionTransformer_Graph(BaseModule):
    """Vision Transformer.
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(VisionTransformer_Graph, self).__init__(init_cfg=init_cfg)
        ...
        self.graph_stage = Sequential(*[
                GraphBlock(
                    in_channels=embed_dims,
                    patch_size=graph_patch_sizes[j // 2]
    ...
    def init_weights(self):
        if (isinstance(self.init_cfg, dict)
            ...
            super(VisionTransformer_Graph, self).init_weights()
        else:
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # B, _, C = x.shape
            # x = x.reshape(B, hw_shape[0], hw_shape[1],
            #               C).permute(0, 3, 1, 2).contiguous()
            # shortcut = x
            # x = self.graph_layers[i](x)
            # x = shortcut + x
            # x = x.permute(0, 2, 3, 1).reshape(B, _, C).contiguous()
    ...
    def train(self, mode=True):
        super(VisionTransformer_Graph, self).train(mode)
