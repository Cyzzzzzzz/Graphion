...
class SwinBlockSequence(BaseModule):
    ...
    def forward(self, x, hw_shape):
        ...
        B, L, C = x.shape
        H, W = hw_shape
        x = x.view(B, H, W, C).permute([0, 3, 1, 2]).clone().contiguous()  # B, C, H, W / L == H * W
        x = self.graph_stage(x)
        x = x.permute([0, 2, 3, 1]).clone().contiguous().view(B, L, C)
        x = shortcut + x
...
class SwinTransformer_Graph(BaseModule):
    ...
    def __init__(self,
        ...
        super(SwinTransformer_Graph, self).__init__(init_cfg=init_cfg)
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformer_Graph, self).train(mode)
        self._freeze_stages()
