...
class SwinTransformer_Graph(BaseBackbone):
    """Swin Transformer.
    ...
    def init_weights(self):
        super(SwinTransformer_Graph, self).init_weights()
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, *args,
        ...
        if (version is None
                or version < 2) and self.__class__ is SwinTransformer_Graph:
            ...
            for k in state_dict_keys:
                if k.startswith('norm.') or k.startswith('backbone.norm.'):
                    ...
                    del state_dict[k]
        if (version is None
                or version < 3) and self.__class__ is SwinTransformer_Graph:
            state_dict_keys = list(state_dict.keys())
            for k in state_dict_keys:
                if 'attn_mask' in k:
                    del state_dict[k]
