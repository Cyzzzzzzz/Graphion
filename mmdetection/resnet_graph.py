...
class GraphBlock(BaseModule):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i in range(N):
            # #-------------------------------Remove-------------------------------#
            # remove_edge = []
            # for j in range(int(self.drop_edge_ratio * num_edges)):
            #     remove_edge.append(int((num_edges - 1) * random.random()))
            # rg.remove_edges(remove_edge)

            #-------------------------------ConbineGraph-------------------------------#
            edges = rg.edges()
            total_edges_u = edges[0] if total_edges_u == None else torch.cat([total_edges_u, edges[0] + total_nodes])
            total_edges_v = edges[1] if total_edges_v == None else torch.cat([total_edges_v, edges[1] + total_nodes])
            total_nodes += rg.num_nodes()
        ...
        #-------------------------------Remove-------------------------------#
        remove_edge = []
        for j in range(int(self.drop_edge_ratio * all_num_edges)):
...
class ResNet_Graph(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(ResNet_Graph, self).__init__(init_cfg)
        ...
        patch_sizes = [8, 4, 2, 1]
        graph_blocks = [3, 4, 6, 3]

        self.res_layers = []
        self.graph_stages = nn.ModuleList()
        self.in_graphs = nn.ModuleList()
        self.out_graphs = nn.ModuleList()
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            in_graph = Sequential(*[
                ...
                nn.BatchNorm2d(num_features=self.inplanes // 2)
            ])
            
            out_graph = Sequential(*[
                torch.nn.Conv2d(
                    self.inplanes // 2,
                    self.inplanes, 
                    kernel_size=1
            ...
            self.in_graphs.append(in_graph)
            self.out_graphs.append(out_graph)
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            x = res_layer(x)
            shortcut = x
            x = self.in_graphs[i](x)
            if x.isnan().sum() > 0 or x.isinf().sum() > 0:
                print("in_graph:", end="\t")
                print(x.isnan().sum(), end="\t")
                print(x.isinf().sum())
            x = self.leak_relu(x)
            x = self.graph_stages[i](x)
            if x.isnan().sum() > 0 or x.isinf().sum() > 0:
                print("graph_stages:", end="\t")
                print(x.isnan().sum(), end="\t")
                print(x.isinf().sum())
            x = self.out_graphs[i](x)
            if x.isnan().sum() > 0 or x.isinf().sum() > 0:
                print("out_graphs:", end="\t")
                print(x.isnan().sum(), end="\t")
                print(x.isinf().sum())
            x = self.leak_relu(x)
            x = shortcut + x
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(ResNet_Graph, self).train(mode)
        self._freeze_stages()
...
@BACKBONES.register_module()
class ResNetV1d_Graph(ResNet_Graph):
    ...
    def __init__(self, **kwargs):
        super(ResNetV1d_Graph, self).__init__(
